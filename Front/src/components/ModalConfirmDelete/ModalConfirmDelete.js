...
class ModalConfirmDelete extends Component {
  ...
  handleSubmit(e) {
    ...
    let params = e.target.getAttribute('data-noticeID');
    ...
    .then((response) => {
      this.handleClose();
      this.props.onNoticeDeleted(response.data.result);
  ...
  render() {
    return (
      ...
      >
        ...
        <Modal.Content>
          <p>Are you sure you want to delete <strong>{this.props.notice.title}</strong>?</p>
        </Modal.Content>
        <Modal.Actions>
          <Button onClick={this.handleSubmit} data-noticeID={this.props.notice._id} color='red'>Yes</Button>
